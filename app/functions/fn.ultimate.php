...
function fn_init_store_params_by_host(&$request, $area = AREA)
{
    ...
    if (!empty($request['switch_company_id']) && $request['switch_company_id'] != 'all' && !isset($request['skip_config_changing'])) { // theme for company with id = 0 cannot be loaded.
        ...
        return array(INIT_STATUS_FAIL, '', 'No storefronts defined');
    }
...
function fn_clone_layouts($data, $from, $to)
{
    ...
    $logo_ids = db_get_hash_multi_array("SELECT company_id, logo_id, type FROM ?:logos WHERE layout_id = 0 and company_id IN (?n)", array('company_id', 'type', 'logo_id'), array($from, $to));

    foreach ($logo_ids[$from] as $type => $logo_id) {
        if (empty($logo_ids[$to][$type])) {
            continue; // workaround if we're in free mode and cloning storefront created in full mode
        }
        fn_clone_image_pairs($logo_ids[$to][$type], $logo_id, 'logos');
    }
